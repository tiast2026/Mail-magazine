...
提案を OK されたら、必要な情報だけ追加で聞く：
- 商品が足りない場合 → 「テンプレB なら3点必要、追加品番ありますか？」
- 配信日時が未定 → 「5/7（木）20:00 配信が定番、それでOK？」
- バナー画像が必要なテンプレ（B/C/I）→ 「セールバナー画像URLありますか？無ければ商品画像で代用も可」

#### Step 6: 即生成 → push

情報揃ったら確認なしで生成 → outputs.json 追記 → commit → push → URL レポート
...
すでに具体的な指示（**品番 + 用途 + テンプレやイベントの指定が揃っている**もの）が
来ている場合のみ、提案フェーズはスキップして即生成に進む。

例：
- ✅ スキップしていい：「品番 abc-1 で **マラソン向けセール告知** 作って」「品番 abc, def, ghi で **テンプレB** で」
- ❌ スキップNG（提案する）：「品番 abc-1 でメルマガ作って」「メルマガ作って」「品番 abc, def」

つまり**品番だけ**渡されても、用途・イベント・テンプレは提案で決める。
